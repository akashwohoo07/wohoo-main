            ...
            {(form.transportMode === "flight" || form.transportMode === "train") && (
              <div className="flex gap-1 bg-zinc-100 p-1 rounded-xl">
                <button onClick={() => setTransportSearch("manual")}
                  className={`flex-1 text-xs font-semibold py-1.5 rounded-lg transition-all ${transportSearch === "manual" ? "bg-white text-zinc-800 shadow-sm" : "text-zinc-500"}`}>
                  ✏️ Manual
                </button>
                {form.transportMode === "flight" && (
                  <button onClick={() => setTransportSearch("flight")}
                    className={`flex-1 text-xs font-semibold py-1.5 rounded-lg transition-all ${transportSearch === "flight" ? "bg-white text-sky-700 shadow-sm" : "text-zinc-500"}`}>
                    ✈️ By flight no.
                  </button>
                )}
                {form.transportMode === "train" && (
                  <button onClick={() => setTransportSearch("train")}
                    className={`flex-1 text-xs font-semibold py-1.5 rounded-lg transition-all ${transportSearch === "train" ? "bg-white text-amber-700 shadow-sm" : "text-zinc-500"}`}>
                    🚂 Search train
                  </button>
                )}
              </div>
